...
<button class="btn btn-danger btn-circle btn-lg fix-btn" id="clear"  data-toggle="tooltip" title="Clear Loyalty Logs">
...
<div class="box" id="main-list">
    <div class="box-header">
        ...
        <button class="btn btn-info pull-right" id="downloadLogs"  data-toggle="tooltip" title="Upload Prices">Download Loyalty Logs</button>
    </div>
    <!-- /.box-header -->
    <div class="box-body">
        <table id="loyalty_logsList" class="table table-bordered table-striped">
        <thead>
        <tr>
            <th>ID</th>
            <th>Log</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             ...
             <h3 class="modal-title">Delete All Loyalty Logs</h3>
            </div>
            <div class="modal-body">
                <center><h4>Are you sure to delete all Loyalty Logs?</h4></center>
            </div>
...
<!-- /.modal -->
<div class="modal fade" id="loyalty_logsDetailsModal">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             ...
             <h3 class="modal-title">Download Loyalty Logs</h3>
            </div>
            <div class="modal-body">
                <center>
            ...
            <div class="modal-footer">
            <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
            <button type="button" class="btn btn-primary" id="downloadBtn">Download Loyalty Logs</button>
            </div>
        </div>
    <!-- /.modal-content -->
    </div>
...
<script>
    ...
    var table = $('#loyalty_logsList').DataTable({ 
        "order": [[ 2, "desc" ]] ,
        'autoWidth'   : true,
        "processing" : true,
        "serverSide" : true, 
        "ajax" : "<?php echo base_url()."portal/loyalty_logs/get_loyalty_logs_list";?>",
    ...
    var main = function(){
        ...
        $("#deleteLogs").click(function(){
            ...
            $.ajax({
                url: "<?php echo base_url();?>portal/loyalty_logs/delete_all_loyalty_logs",
                type: "post",
                data: values ,
                success: function (response) {
                    toastr.success("All loyalty_logs successfully deleted");
        ...
        $("#downloadBtn").click(function(){
            window.open ("<?php echo base_url()."portal/loyalty_logs/download?fromDate="?>" + startDateDownload + "&toDate=" + endDateDownload,"_blank") ;
        });
    }
    ...
    function _view(id)
    {
        var values = { "id": id };
        $.ajax({
            url: "<?php echo base_url();?>portal/loyalty_logs/get_loyalty_log_details",
            type: "post",
            data: values ,
            success: function (response) {
                ...
                $("#loyalty_logsDetailsModal").modal("show");
            },
            error: function(jqXHR, textStatus, errorThrown) {
            console.log(textStatus, errorThrown);
            $("#deleteLogs").button("reset");
