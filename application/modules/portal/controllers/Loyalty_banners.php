...
class Loyalty_banners extends CI_Controller {
    public function __construct()
    {
        parent::__construct();
        $this->settings_model->get_settings();   
        $this->load->model("portal/loyalty_banners_model"); 
	...
	public function add_loyalty_banner()
	{
        $this->loyalty_banners_model->banner_image = $this->input->post("banner_image");
        $this->loyalty_banners_model->title = $this->input->post("title");
        $this->loyalty_banners_model->description = $this->input->post("description");
        $this->loyalty_banners_model->link = $this->input->post("link");
        $this->loyalty_banners_model->status = $this->input->post("status");
        echo $this->loyalty_banners_model->insert_loyalty_banners();
	} 

	public function edit_loyalty_banner()
	{
        ...
        $this->loyalty_banners_model->link = $this->input->post("link");
        $this->loyalty_banners_model->status = $this->input->post("status");
        $this->loyalty_banners_model->id = $banners_id;
        echo $this->loyalty_banners_model->update_loyalty_banners();
	}

	public function delete_loyalty_banner()
	{
        ...
        $this->db->select("id,title,description,banner_image,link,created_by,date_created,date_modified,modified_by,content,status");
         $data_banners = $this->db->get("loyalty_banners");
        $this->db->where("id",$id);
        echo $result = $this->db->delete("loyalty_banners");
    ...
    public function get_banner_data()
    {
        $id = $this->input->post("id");
        $this->db->where("id",$id);
        $result = $this->db->get("loyalty_banners");
        $banners = $result->row();
        ...
        $return["loyalty_banners"] = $banners;
        echo json_encode($return); 
    }

    public function get_loyalty_banners_list()
    {
        ...
        $this->dt_model->table = "loyalty_banners AS t1 LEFT JOIN user_accounts AS t2 ON t2.id = t1.created_by LEFT JOIN user_accounts AS t3 ON t3.id = t1.modified_by LEFT JOIN media as t4 on t4.id = t1.banner_image";  
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            foreach ($select_columns as $col) {
                    ...
                    else if($col == "banner_image")
                    {
                        ...
                        {  
                            $row[] = "<a href=\"#\" onclick='return false;'><img class='img-thumbnail' src='".base_url()."uploads/loyalty_banners/".$aRow[$col]."' style='height:70px;' onclick='img_preview(\"".$aRow[$col]."\")'></a>";
                        }
                        else
                        {
                            $row[] = "None";
